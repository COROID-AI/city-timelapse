...
function hashEra(eraId: EraId): number {
...
function clampSample(x: number) {
  // Soft clip to prevent harsh clipping.
  return Math.tanh(x * 2) * 0.5
}

function generateEraBuffer({
  eraId,
  sampleRate,
  seconds,
}: {
  ...
  const times: {
    t: number
    amp: number
    type: 'clink' | 'beep' | 'ring' | 'click' | 'notif'
  }[] = []

  const addRandomTimes = (
    count: number,
    minT: number,
    maxT: number,
    ampBase: number,
  ...
  for (let i = 0; i < len; i++) {
    ...
    switch (eraId) {
      ...
      case '1965': {
        ...
        const riff =
          0.35 * Math.sin(twoPi * (baseF * 2) * tc) +
      ...
      case '1985': {
        ...
        const synth =
          (Math.sin(twoPi * freq * tc) * 0.35 + Math.sin(twoPi * freq2 * tc + 0.7) * 0.25) * 0.08 * gate
        sample += synth
        break
      }
      case '2005': {
        ...
        const chord =
          Math.sin(twoPi * chordBase * tc) * 0.05 + Math.sin(twoPi * chordBase * 2 * tc + 0.4) * 0.02
    ...
    for (let j = 0; j < times.length; j++) {
      ...
      if (wrappedDt < 0 || wrappedDt > 0.25) continue
...
class SfxMixerImpl {
  ...
  private pendingDurationMs = 1500
  private unlocked = false

  private muted = false
  private defaultMasterGain = 0.65

  private buffers = new Map<EraId, AudioBuffer>()
  ...
  subscribeMute(listener: (muted: boolean) => void) {
  ...
  private ensureAudioContext(sampleRate?: number) {
    ...
    const AudioCtx =
      (window.AudioContext || (window as any).webkitAudioContext) as unknown as {
        new (opts: { sampleRate?: number }): AudioContextLike
      }
  ...
  private getOrCreateBuffer(eraId: EraId, sampleRate: number) {
  ...
  setEra(eraId: EraId, durationMs = 1500) {
    if (!this.unlocked) {
    ...
    if (this.currentEraId === eraId && this.currentSource && this.currentGain) return
