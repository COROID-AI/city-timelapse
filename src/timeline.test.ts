...
describe('TimelineSlider', () => {
  ...
  it('renders exactly five ticks with the plan labels', () => {
    const { slider } = createHarness()
    const labels = Array.from(
      slider.root.querySelectorAll<HTMLElement>('.timeline-era-label'),
    ).map((el) => el.textContent)
    expect(labels).toEqual(['1945', '1965', '1985', '2005', '2025'])
    expect(labels).not.toContain('2055')
  })
  ...
  it('dragging the thumb selects eras from pointer position', () => {
    ...
    // 10/400 ≈ 0.025 → 1945 (no change); 200/400 = 0.5 → index 2 → 1985.
    expect(slider.value).toBe('1985')
    expect(changes.map((c) => c.era)).toEqual(['1985'])

    // A second drag to the far right lands on 2025.
    track.dispatchEvent(
  ...
  it('supports all five eras via keyboard', () => {
    const { slider } = createHarness()
    const thumb = slider.root.querySelector<HTMLElement>('.timeline-thumb')!
    const press = (key: string) =>
      thumb.dispatchEvent(
