...
import { MeshStandardMaterial, CylinderGeometry, BoxGeometry, SphereGeometry, ConeGeometry, TorusGeometry, Color, } from 'three';
/**
 ...
 */
const TablewareItem = ({ era, eraData }) => {
 ...
 */
const TablewarePrimitive = ({ type, position, rotation, size, era, }) => {
    ...
    return (_jsx("mesh", { position: position, rotation: rotation, receiveShadow: true, castShadow: true, children: _jsx("primitive", { geometry: geometry, material: threeMaterial }) }));
};
/**
 ...
 */
const LightingFixture = ({ type, position, rotation, size, era, eraData, }) => {
 ...
 */
export const TablewareLighting = () => {
    const { currentEra, eraData } = useStore();
    const era = currentEra || '1945';
    useEffect(() => {
 ...
 */
const renderTableware = (era, eraData) => {
    const tablewareItemsPerEra = {
        '1945': ['tinCup', 'ceramicMug', 'paperNapkin', 'metalCutlery'],
        '1965': ['colorfulCeramicMug', 'glassJuiceGlass', 'clothNapkin', 'chromeCutlery'],
        '1985': ['plasticCupWithStraw', 'brandedCeramicMug', 'paperNapkinWithLogo', 'plasticCutlery'],
    ...
    return items.map((item, index) => (_jsx(TablewarePrimitive, { type: item, position: positions[index % positions.length], rotation: { x: 0, y: 0, z: 0 }, size: 1.0, era: era }, item)));
};
/**
 * Render lighting fixtures for the current era using Three.js primitives
 * Fixtures are placed on ceiling/wall positions
 */
const renderLighting = (era, eraData) => {
    const lightingFixturesPerEra = {
        '1945': ['incandescentPendant', 'gasLampSconce'],
        '1965': ['sputnikChandelier', 'coloredBulbAccent', 'trackLighting'],
        '1985': ['fluorescentPanel', 'neonAccentStrip', 'discoBall'],
    ...
    return fixtures.map((fixture, index) => (_jsx(LightingFixture, { type: fixture, position: positions[index % positions.length], rotation: { x: 0, y: 0, z: 0 }, size: 1.5, era: era, eraData: eraData }, fixture)));
};
