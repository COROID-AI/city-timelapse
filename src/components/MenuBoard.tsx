...
import { VisualEraData, MenuItem } from '../eras';
...
export const MenuBoard: React.FC = () => {
  ...
  const eraData = VisualEraData[era as keyof typeof VisualEraData];
  ...
  return (
    ...
    >
      <div style={{ fontWeight: 'bold', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '1px' }}>
        {era}: MENU BOARD
      </div>
      <div style={{ margin: '4px 0', display: 'flex', justifyContent: 'space-between' }}>
        {eraData.menuItems.map((item, index) => (
          <div
            key={index}
            style={{
              flex: '1',
          ...
          >
            {item.name}: ${item.price.toFixed(
              era === '1945' ? 2 : era === '1965' ? 2 : era === '1985' ? 2 : era === '2005' ? 2 : 2,
            )}
          </div>
        ))}
      </div>
      {/* Era-specific notes */}
      {era === '1945' && (
        <div style={{ fontSize: '10px', marginTop: '4px', color: '#888', fontStyle: 'italic' }}>
          Wartime rationing: Coffee limited per adult
        </div>
      )}
      {era === '1965' && (
        <div style={{ fontSize: '10px', marginTop: '4px', color: '#666', fontStyle: 'italic' }}>
          Beatles-era café references
        </div>
      )}
      {era === '2025' && (
        <div style={{ fontSize: '10px', marginTop: '4px', color: '#666', fontStyle: 'italic' }}>
          QR code ordering available
        </div>
      )}
    </div>
  );
