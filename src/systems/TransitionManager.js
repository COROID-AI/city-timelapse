 ...
 */
const interpolate = (currentVal, targetVal, progress) => {
 ...
 */
const interpolateNumber = (current, target, progress) => {
    return current + (target - current) * easeInOutCubic(progress);
};
/**
 * Interpolate fog density
 */
const interpolateFogDensity = (current, target, progress) => {
    return current + (target - current) * easeInOutCubic(progress);
};
/**
 * Interpolate ambient light intensity
 */
const interpolateAmbientIntensity = (current, target, progress) => {
    return current + (target - current) * easeInOutCubic(progress);
};
/**
 * Interpolate lighting color temperature
 */
const interpolateLightingColorTemp_single = (current, target, progress) => {
    return current + (target - current) * easeInOutCubic(progress);
};
/**
 * Interpolate ambient light color (hex) between two eras
 */
const interpolateAmbientLightColor = (currentColor, targetColor, progress) => {
    const easedProgress = easeInOutCubic(progress);
 ...
 */
const interpolateLightingColorTemp_between = (currentTemp, targetTemp, progress) => {
 ...
 */
const lightingTypeToColorTemp = (lightingType) => {
    switch (lightingType) {
        case 'warm_glow':
 ...
 */
export const useEraTransition = () => {
    const store = useEraStore();
    const { currentEra, targetEra, isTransitioning, transitionProgress, transitionDuration, onTransitionComplete, setTransitionDuration, resetTransition, } = store;
    ...
    const [lightingColorTemp, setLightingColorTemp] = useState(6500);
        ...
        const updateInterpolatedProps = () => {
            ...
            if (!isTransitioning || !targetEraData) {
                ...
                if (currentEraData) {
                    ...
                    const currentTemp = lightingTypeToColorTemp(getLightingType(currentEra));
                    ...
                    const targetTemp = targetEraData
                        ? targetEraData.lightingType
                            ...
                            : currentTemp
                        : currentTemp;
                    setLightingColorTemp(interpolateLightingColorTemp_single(currentTemp, targetTemp, 0));
            ...
            const interpolatedColor = interpolate(currentEraData.wallColor, targetEraData.wallColor, progress);
            ...
            const interpolatedFog = interpolateFogDensity(currentEraData.fogDensity, targetEraData.fogDensity, progress);
            setFogDensity(interpolatedFog);
            // Ambient light color interpolation
            const interpolatedAmbientColor = interpolateAmbientLightColor(currentEraData.ambientLightColor, targetEraData.ambientLightColor, progress);
            setAmbientLightColor(interpolatedAmbientColor);
            // Lighting color temperature interpolation
            const currentTemp = lightingTypeToColorTemp(getLightingType(currentEra));
            const targetTemp = lightingTypeToColorTemp(getLightingType(targetEra));
            const interpolatedColorTemp = interpolateLightingColorTemp_between(currentTemp, targetTemp, progress);
    ...
    const interruptTransition = useCallback((newEraId) => {
        if (currentEra === newEraId)
            return;
